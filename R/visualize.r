  ...
  if (use_dot) {
    ...
    if (levelwise) {
      ...
      subgraphs <- lapply(1:max(df_lev$.level), function(l) {
        nodes <- as.character(df_lev[df_lev$.level == l, ".index"])
    ...
    igraph::plot.igraph(btg$graph,
      layout = btg$layout, vertex.label = as.character(labels),
