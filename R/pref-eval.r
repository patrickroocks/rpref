...
#'
#' @param df A data frame, data frame extension (e.g. a tibble), or a grouped data frame from \code{\link[dplyr]{group_by}}.
#' @param pref A preference object. See \code{\link{complex_pref}} and \code{\link{base_pref}} for details.
...
psel <- function(df, pref, ...) {
  ...
  show_level <- get.bool.from.lst(vars, "show_level", is_top)
  vars$show_level <- show_level

  show_index <- get.bool.from.lst(vars, "show_index", FALSE)

  # Remove show_indices from vars, not needed in psel.indices
  vars <- vars[!names(vars) %in% c("show_index")]

  # Call psel.indices with all parameters from ...
  tmp_res <- psel.indices(df, pref, .dots = vars)

  # Extract indices
  if (!show_level) {
    indices <- tmp_res
  } # psel.indices.top returned just indices when show_level = FALSE
  else {
    indices <- tmp_res[[".index"]]
    levels <- tmp_res[[".level"]]
  }

  # Apply indices (potential grouping is preserved!)
  res <- df[indices, , drop = FALSE]

  # Add indices if show_indices is true
  if (show_index) {
    res[[".index"]] <- indices
  }
...
psel.indices <- function(df, pref, ...) {
  ...
  # Show-level operates independent of is_top!
  show_level <- get.bool.from.lst(vars, "show_level") # FALSE by Default (for psel.indices)

  if (is_top) {
    # Logical options
    and_connected <- get.bool.from.lst(vars, "and_connected", TRUE)

    num_rows <- nrow(df)

    # No defaults, but value -1 if null
    top <- get.top.param.from.lst(vars, "top", num_rows)
    top_level <- get.top.param.from.lst(vars, "top_level", num_rows)
    at_least <- get.top.param.from.lst(vars, "at_least", num_rows)
  ...
  if (!is_top) {
    ...
    if (!show_level) { # just return indices
      ...
      return(data.frame(.index = res + 1, .level = 1))
    }
  } else {
    # Do the top-k preference selection
    if (!is_grouped) { # Usual preference selection (not grouped)
      res <- pref_select_top_impl(
        scores, pref_serial, Npar, alpha,
        top, at_least, top_level, and_connected, show_level
      )
    } else { # Grouped preference selection
      res <- grouped_pref_sel_top_impl(
        group_indices, scores, pref_serial, Npar, alpha,
        top, at_least, top_level, and_connected, show_level
      )
    }
    ...
    return(res) # Return Data.Frame(".index", ".level")
  }
}
...
# Helper for top-k parameters
get.top.param.from.lst <- function(lst, name, inf_default) {
  ...
  val <- lst[[name]]
  if (is.null(val)) {
    stop.syscall(paste0("Parameter ", name, "must be a positive single integer value, got NULL"))
  } else if (length(val) != 1) {
    stop.syscall(paste0("Parameter ", name, " must be a positive single integer value, got value of length ", length(val)))
  } else if (!is.numeric(val)) {
    stop.syscall(paste0("Parameter ", name, " must be a positive single integer value, got value of type ", typeof(val)))
  } else if (round(val) != val || val < 0 || is.na(val)) {
    stop.syscall(paste0("Parameter ", name, " must be a positive single integer value, got value ", val))
  } else if (val == Inf) {
    return(inf_default)
  } else {
    return(val)
...
# Helper for logicals
get.bool.from.lst <- function(lst, name, default = FALSE) {
  if (!(name %in% names(lst))) {
    # Default value
    return(default)
  }
  val <- lst[[name]]
  if (is.null(val)) {
    stop.syscall(paste0("Parameter ", name, "must be a single logical value, got NULL"))
  } else if (length(val) != 1) {
    stop.syscall(paste0("Parameter ", name, " must be a single value, got value of length ", length(val)))
  } else if (!is.logical(val)) {
    stop.syscall(paste0("Parameter ", name, " must be a single logical value, got value of type ", typeof(val)))
  } else if (is.na(val)) {
    stop.syscall(paste0("Parameter ", name, " must be a single logical value, got value NA"))
  } else {
    return(val)
  }
}
...
df.pref.check <- function(df, pref) {
  # Note that also data.table or tibble fulfills is.data.frame(.) = TRUE
  if (!is.data.frame(df)) stop.syscall("First argument has to be a data frame or a data frame extension.")
  if (!is.actual.preference(pref)) stop.syscall("Second argument has to be a preference.")
}

stop.syscall <- function(message) {
  message <- paste0("Error in ", deparse(sys.calls()[[sys.nframe() - 2]]), " : ", message)
