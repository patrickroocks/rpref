...
#'
#'
#' \item A query in the syntax from Kießling (2002) like
#'
...
#' @references
#'
#' S. Börzsönyi, D. Kossmann, K. Stocker (2001): The Skyline Operator. In Data Engineering (ICDE '01), pages 421-430.
#'
#' W. Kießling (2002): Foundations of Preferences in Database Systems. In Very Large Data Bases (VLDB '02), pages 311-322.
#'
...
#' @docType methods
setMethod(
  "*", signature(e1 = "preference", e2 = "preference"),
  function(e1, e2) {
    check_pref(e1, e2)
    if (check_empty(e1, e2)) {
      return(get_empty(e1, e2))
    }
...
"-.preference" <- function(p1, p2) {
  if (nargs() == 1) {
    return(reverse(p1))
  } # calls reverse from above
  else {
    stop("Operation not defined.")
  }
}
