...
// M.Endres, P.Roocks, W.KieÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¸ling
...
bool scalagon::get_prefs(ppref& p) {
...
list<int> scalagon::run(vector<int>& v, ppref& p, double alpha) {
  
  if (init(v, p, alpha)) { // return false if input does not suit
    
    // *** Domination phase
    vector<int> v_empty; // Create empty vector to allow reference
    dominate(v_empty, 0, p); // empty vecor - no index needed for non-topk scalagon
...
flex_list scalagon::run_topk(vector<int>& v, ppref& p, topk_setting& ts, double alpha, bool show_levels) {

	list<int> final_result_list;
	pair_list final_result_pair_list; // Pairs of level and tuple index

	if (ts.is_simple) {
		//return std scalagon WITHOUT levels - finalt_result_pair_list is empty!
		if (!show_levels) return(flex_list(run(v, p, alpha),  final_result_pair_list)); 
		else              return(flex_list(final_result_list, bnl_alg.add_level(run(v, p, alpha), 1))); // ... WITH LEVELS, final_result_list is empty
	}
	...
	if (init(v, p, alpha)) { // use Scalagon
		...
		for (int i = 0; i < m_filt_count; i++) index_pairs[i] = pair<int, int>(m_filt_res[i], -1); // outliers => they have no s-index
		...
		while (true) {
			...
			// *** Domination phase
			dominate(s_indices, s_ind_count, p);
			...
			for (int i = 0; i < s_ind_count; i++) {
				...
				if (!m_btg[btg_ind]) {
					...
					index_pairs[paircount] = pair<int, int>(v[m_stuples_v[cur_s_sind]], cur_s_sind);
					paircount++;
				} else {
					// not in Scalagon filtered tuples => add to remainder
					remainder_pairs[remcount] = pair<int, int>(v[m_stuples_v[cur_s_sind]], cur_s_sind);
					remcount++;
				}
			}
			...
			pair_list res = bnl_alg.run_remainder_paired(index_pairs, paircount, remainder_pairs, remcount, p);
			...
			for (pair_list::iterator j = res.begin(); j != res.end(); ++j)
				if (show_levels) final_result_pair_list.push_back(pair<int, int>(level, j->first)); // Add level
		...
		if (show_levels) final_result_pair_list = bnl_alg.run_topk_lev(v, p, ts);
		else final_result_list =                  bnl_alg.run_topk(v, p, ts);
	}
...
inline int scalagon::get_index_pt(vector<int> &pt) {
  int res = pt[0];
...
vector<int> scalagon::iterated_scaling(vector<int>& domain_size, const double btg_size) {
...
bool scalagon::init(vector<int>& v, ppref& p, double alpha) {
...
void scalagon::dominate(vector<int>& s_ind, int scount, ppref& p) {
  ...
  bool use_ind = scount > 0; // is only true if s_ind is set - just for topk!
  if (!use_ind) scount = m_stuples_v.size();
